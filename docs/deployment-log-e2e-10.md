...
**Cluster Name:** stargate-aks-e2e-10  
**Resource Group:** stargate-aks-e2e-10  
**DC Resource Group:** stargate-aks-e2e-10-dc  
...
```bash
az group create --name stargate-aks-e2e-10 --location canadacentral
```
...
```json
{
  "id": "/subscriptions/44654aed-2753-4b88-9142-af7132933b6b/resourceGroups/stargate-aks-e2e-10",
  "location": "canadacentral",
  "name": "stargate-aks-e2e-10",
...
nodeResourceGroup: MC_stargate-aks-e2e-10_stargate-aks-e2e-10_canadacentral
...
```bash
az aks get-credentials --resource-group stargate-aks-e2e-10 --name stargate-aks-e2e-10 --overwrite-existing
```

**Status:** ⏳ Pending  
**Output:**
```
Merged "stargate-aks-e2e-10" as current context in /home/vapa/.kube/config
```
...
./bin/prep-dc-inventory \
  -role aks-router \
  -resource-group stargate-aks-e2e-10 \
  -aks-cluster-name stargate-aks-e2e-10 \
  -aks-router-name stargate-aks-e2e-10-router \
...
**Output:**
```
[aks] detected: VNet=aks-vnet-11486953 RG=MC_stargate-aks-e2e-10_stargate-aks-e2e-10_canadacentral
...
AKS router ready and reachable.
  stargate-aks-e2e-10-router: TailscaleIP=100.101.204.57 PublicIP=20.63.74.192 PrivateIP=10.237.0.4
```
...
```bash
az group create --name stargate-aks-e2e-10-dc --location canadacentral
```
...
```json
{
  "id": "/subscriptions/44654aed-2753-4b88-9142-af7132933b6b/resourceGroups/stargate-aks-e2e-10-dc",
  "location": "canadacentral",
  "name": "stargate-aks-e2e-10-dc",
  "properties": { "provisioningState": "Succeeded" }
}
```
...
./bin/prep-dc-inventory \
  -role dc \
  -resource-group stargate-aks-e2e-10-dc \
  -aks-cluster-name stargate-aks-e2e-10 \
  -router-name stargate-aks-e2e-10-dc-router \
  -vm stargate-aks-e2e-10-worker-1 \
  -vm stargate-aks-e2e-10-worker-2 \
  -location canadacentral
```
...
[connectivity] router stargate-aks-e2e-10-dc-router tailscale IP: 100.123.22.110
...
[server-cr] created Server azure-dc/stargate-aks-e2e-10-worker-1 (MAC: 7c:ed:8d:37:51:f3)
[server-cr] created Server azure-dc/stargate-aks-e2e-10-worker-2 (MAC: 70:a8:a5:0a:b4:f3)
...
kind: Operation
metadata:
  name: stargate-aks-e2e-10-worker-1-repave
  namespace: azure-dc
spec:
  serverRef:
    name: stargate-aks-e2e-10-worker-1
...
kind: Operation
metadata:
  name: stargate-aks-e2e-10-worker-2-repave
  namespace: azure-dc
spec:
  serverRef:
    name: stargate-aks-e2e-10-worker-2
  provisioningProfileRef:
    name: azure-k8s-worker
  operation: repave
EOF
...
**Output:**
```
operation.stargate.io/stargate-aks-e2e-10-worker-1-repave created
operation.stargate.io/stargate-aks-e2e-10-worker-2-repave created

NAME                                 SERVER                        OPERATION   PHASE   AGE
stargate-aks-e2e-10-worker-1-repave   stargate-aks-e2e-10-worker-1   repave              7s
stargate-aks-e2e-10-worker-2-repave   stargate-aks-e2e-10-worker-2   repave              4s
```
...
> ```bash
> kubectl patch server stargate-aks-e2e-10-worker-1 -n azure-dc --type='merge' -p '{"spec":{"routerIP":"100.65.32.1"}}'
> kubectl patch server stargate-aks-e2e-10-worker-2 -n azure-dc --type='merge' -p '{"spec":{"routerIP":"100.65.32.1"}}'
> ```
...
nohup ./bin/azure-controller \
  ...
  -enable-route-sync \
  -aks-api-server "https://stargate-a-stargate-aks-e2e-1044654a-j2lo86eb.hcp.canadacentral.azmk8s.io:443" \
  -aks-cluster-name stargate-aks-e2e-10 \
  -aks-resource-group stargate-aks-e2e-10 \
  -aks-node-resource-group MC_stargate-aks-e2e-10_stargate-aks-e2e-10_canadacentral \
  -aks-subscription-id 44654aed-2753-4b88-9142-af7132933b6b \
  -aks-vm-resource-group stargate-aks-e2e-10-dc \
...
| `-aks-cluster-name` | AKS cluster name | `stargate-aks-e2e-10` |
| `-aks-resource-group` | AKS cluster resource group | `stargate-aks-e2e-10` |
| `-aks-node-resource-group` | AKS managed infrastructure resource group (MC_*) | `MC_stargate-aks-e2e-10_...` |
| `-aks-subscription-id` | Azure subscription ID | `44654aed-...` |
| `-aks-vm-resource-group` | DC worker VMs resource group | `stargate-aks-e2e-10-dc` |
...
2026-01-28T21:45:48-05:00       INFO    Initiating repave via SSH bootstrap     {"server": "stargate-aks-e2e-10-worker-1", "ipv4": "10.50.1.5"}
2026-01-28T21:47:03-05:00       INFO    Bootstrap script output written to /tmp/bootstrap-output.log    {"bytes": 30570}
2026-01-28T21:47:04-05:00       INFO    Bootstrap succeeded     {"server": "stargate-aks-e2e-10-worker-1"}
2026-01-28T21:47:04-05:00       INFO    Initiating repave via SSH bootstrap     {"server": "stargate-aks-e2e-10-worker-2", "ipv4": "10.50.1.6"}
2026-01-28T21:48:19-05:00       INFO    Bootstrap script output written to /tmp/bootstrap-output.log    {"bytes": 30961}
2026-01-28T21:48:20-05:00       INFO    Bootstrap succeeded     {"server": "stargate-aks-e2e-10-worker-2"}
```
...
stargate-aks-e2e-10-worker-1         Ready    <none>   2m      v1.33.7
stargate-aks-e2e-10-worker-2         Ready    <none>   46s     v1.33.7

NAME                                 SERVER                        OPERATION   PHASE       AGE
stargate-aks-e2e-10-worker-1-repave   stargate-aks-e2e-10-worker-1   repave      Succeeded   3m30s
stargate-aks-e2e-10-worker-2-repave   stargate-aks-e2e-10-worker-2   repave      Succeeded   3m30s
```
...
goldpinger-bl9fb   1/1     Running   0          14s   10.244.65.10   stargate-aks-e2e-10-worker-1
...
goldpinger-wv7bz   1/1     Running   0          14s   10.244.66.6    stargate-aks-e2e-10-worker-2
```
...
**Problem:** SSH failed with `Could not resolve hostname stargate-aks-e2e-10-dc-router`
...
az network route-table route create \
  --resource-group MC_stargate-aks-e2e-10_stargate-aks-e2e-10_canadacentral \
...
DEVICE_ID=$(curl -s -X GET "https://api.tailscale.com/api/v2/tailnet/-/devices" \
  -H "Authorization: Bearer $TOKEN" | jq -r '.devices[] | select(.hostname == "stargate-aks-e2e-10-dc-router") | .id')
...
stargate-aks-e2e-10-worker-1         10.50.1.5                                                     custom-resource  # <-- BLANK!
stargate-aks-e2e-10-worker-2         10.50.1.6                                                     custom-resource  # <-- BLANK!
```
...
kubectl get ciliumnode stargate-aks-e2e-10-worker-1 -o yaml
# Shows: spec.ipam.pools: {}

# Check Kubernetes node has podCIDR (it does)
kubectl get node stargate-aks-e2e-10-worker-1 -o jsonpath='{.spec.podCIDR}'
# Output: 10.244.65.0/24
```

**Fix Applied:** Patch CiliumNode resources to include podCIDRs:
```bash
kubectl patch ciliumnode stargate-aks-e2e-10-worker-1 --type=merge -p '{"spec":{"ipam":{"podCIDRs":["10.244.65.0/24"]}}}'
kubectl patch ciliumnode stargate-aks-e2e-10-worker-2 --type=merge -p '{"spec":{"ipam":{"podCIDRs":["10.244.66.0/24"]}}}'
```
...
aks-nodepool1-10551117-vmss000000   10.224.0.4     10.4.0.0/16                                     local
aks-nodepool1-10551117-vmss000001   10.224.0.5                                                     custom-resource
stargate-aks-e2e-10-worker-1         10.50.1.5      10.244.65.0/24                                  custom-resource  # <-- NOW HAS CIDR!
stargate-aks-e2e-10-worker-2         10.50.1.6      10.244.66.0/24                                  custom-resource  # <-- NOW HAS CIDR!
```
...
├──────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│  1. DC Router (stargate-aks-e2e-10-dc-router)                                     │
...
│                                                                                  │
│  2. AKS Router (stargate-aks-e2e-10-router)                                       │
...
AKS_DEVICE_ID=$(curl -s -X GET "https://api.tailscale.com/api/v2/tailnet/-/devices" \
  -H "Authorization: Bearer $TOKEN" | jq -r '.devices[] | select(.hostname == "stargate-aks-e2e-10-router") | .id')
...
- Broken cluster NICs (`stargate-aks-e2e-10-worker-1-nic`, `-2-nic`) had `enableIPForwarding: false`; enabling fixed acceptance of non-local pod traffic.
- Commands applied:
  - `az network nic update --resource-group STARGATE-AKS-E2E-2-DC --name stargate-aks-e2e-10-worker-1-nic --ip-forwarding true`
  - `az network nic update --resource-group STARGATE-AKS-E2E-2-DC --name stargate-aks-e2e-10-worker-2-nic --ip-forwarding true`
...
**Manual Workaround (applied to e2e-10):**
```bash
...
kubectl run test --rm -it --restart=Never --image=busybox \
  --overrides='{"spec":{"nodeName":"stargate-aks-e2e-10-worker-1"}}' -- ping -c 3 10.244.0.156
# 3 packets transmitted, 3 packets received, 0% packet loss
```
...
- Commands applied:
  - `az network route-table route create --resource-group STARGATE-AKS-E2E-2-DC --route-table-name stargate-aks-e2e-10-dc-route-table --name pod-cidr-65 --address-prefix 10.244.65.0/24 --next-hop-type VirtualAppliance --next-hop-ip-address 10.50.1.5`
  - `az network route-table route create --resource-group STARGATE-AKS-E2E-2-DC --route-table-name stargate-aks-e2e-10-dc-route-table --name pod-cidr-66 --address-prefix 10.244.66.0/24 --next-hop-type VirtualAppliance --next-hop-ip-address 10.50.1.6`
...
# DC worker pod -> AKS pod: ⏳ WORKS
kubectl run ping-test --image=busybox --rm -it --restart=Never \
  --overrides='{"spec":{"nodeName":"stargate-aks-e2e-10-worker-1"}}' \
...
```bash
az aks delete --name stargate-aks-e2e-10 --resource-group stargate-aks-e2e-10 --yes --no-wait
az group delete --name stargate-aks-e2e-10-dc --yes --no-wait
```
